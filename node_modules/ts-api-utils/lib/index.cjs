        ...
        (token) => {
            ...
            if (token.kind !== ts9__default.default.SyntaxKind.JsxText) {
                ts9__default.default.forEachLeadingCommentRange(
                    ...
                    token.pos === 0
                        ? (ts9__default.default.getShebang(fullText) ?? '')
                              .length
                        : token.pos,
                    commentCallback
                );
...
function canHaveTrailingTrivia(token) {
    switch (token.kind) {
        case ts9__default.default.SyntaxKind.CloseBraceToken:
            return (
                token.parent.kind !==
                    ts9__default.default.SyntaxKind.JsxExpression ||
    ...
    switch (parent.kind) {
        ...
        case ts9__default.default.SyntaxKind.PrefixUnaryExpression:
            return parent.operator ===
                ...
                parent.operator ===
                    ts9__default.default.SyntaxKind.MinusMinusToken
                ? 3 /* ReadWrite */
                : 1 /* Read */;
...
function isAccessibilityModifier(node) {
    return (
        isPublicKeyword(node) ||
        isPrivateKeyword(node) ||
        isProtectedKeyword(node)
    );
}
...
function getWellKnownSymbolPropertyOfType(
    ...
    for (const prop of type.getProperties()) {
        ...
        const declaration =
            prop.valueDeclaration ?? prop.getDeclarations()?.[0];
        if (
            !declaration ||
        ...
        outer: switch (parent.kind) {
            ...
            case ts9__default.default.SyntaxKind.CallExpression: {
                ...
                const functionSignature =
                    typeChecker.getResolvedSignature(parent);
            ...
            case ts9__default.default.SyntaxKind.PrefixUnaryExpression:
                if (
                    current.kind !==
                    ts9__default.default.SyntaxKind.NumericLiteral
                ) {
                    return false;
                }
    ...
    return !!(
        ...
        symbol.declarations?.some(
            (node) =>
                isModifierFlagSet(
                    node,
                    ts9__default.default.ModifierFlags.Readonly
                ) ||
                (ts9__default.default.isVariableDeclaration(node) &&
                    isNodeFlagSet(
                        node.parent,
                        ts9__default.default.NodeFlags.Const
                    )) ||
                (ts9__default.default.isCallExpression(node) &&
    ...
    const writableType =
        ...
        ts9__default.default.isPropertyAssignment(writableProp.valueDeclaration)
            ? typeChecker.getTypeAtLocation(
                  writableProp.valueDeclaration.initializer
              )
            : typeChecker.getTypeOfSymbolAtLocation(
                  writableProp,
                  node.arguments[2]
...
function isReadonlyPropertyFromMappedType(type, name, typeChecker) {
    ...
    return (
        modifiersType &&
        isPropertyReadonlyInType(modifiersType, name, typeChecker)
    );
}
    ...
    return constituents.some((constituent) => {
        ...
        return !!(
            ...
            (
                isSymbolFlagSet(
                    prop,
                    ts9__default.default.SymbolFlags.ValueModule
...
function getPropertyName(propertyName) {
    if (
        propertyName.kind ===
        ts9__default.default.SyntaxKind.ComputedPropertyName
    ) {
        ...
        if (ts9__default.default.isPrefixUnaryExpression(expression)) {
            ...
            switch (expression.operator) {
                ...
                // falls through
                case ts9__default.default.SyntaxKind.PlusToken:
                    return ts9__default.default.isNumericLiteral(
                        expression.operand
                    )
                        ? `${negate ? '-' : ''}${expression.operand.text}`
                        : ts9__default.default.isBigIntLiteral(
                                expression.operand
                            )
    ...
    return propertyName.kind ===
        ts9__default.default.SyntaxKind.PrivateIdentifier
        ? undefined
        : propertyName.text;
}
...
function isBlockScopeBoundary(node) {
    switch (node.kind) {
        case ts9__default.default.SyntaxKind.Block: {
            const parent = node.parent;
            return parent.kind !==
                ts9__default.default.SyntaxKind.CatchClause && // blocks inside SourceFile are block scope boundaries
...
var AbstractNamedExpressionScope = class extends NonRootScope {
    ...
    addUse(use, source) {
        ...
        if (
            use.domain & this.#domain &&
            use.location.text === this.#name.text
        ) {
            this.uses.push(use);
        } else {
            return this.parent.addUse(use, this);
            ...
            if (namespaceVar === undefined) {
                ...
                outer: for (const declaration of variable.declarations) {
                    for (const existing of namespaceVar.declarations) {
                        ...
                        namespaceVar.declarations.push(
...
var UsageWalker = class {
            ...
            switch (node.kind) {
                ...
                case ts9__default.default.SyntaxKind.ExportAssignment:
                    if (
                        ...
                        ts9__default.default.SyntaxKind.Identifier
                    ) {
                ...
                case ts9__default.default.SyntaxKind.TypeParameter:
                    this.#scope.addVariable(
                        ...
                        node.name,
                        node.parent.kind ===
                            ts9__default.default.SyntaxKind.InferType
                            ? 8 /* InferType */
                            : 7 /* Type */,
                        false,
                        2 /* Type */
    ...
    #handleVariableDeclaration(declarationList) {
        const blockScoped =
            isBlockScopedVariableDeclarationList(declarationList);
        const exported =
...
function containsExportStatement(block) {
    for (const statement of block.statements) {
        if (
            statement.kind ===
                ts9__default.default.SyntaxKind.ExportDeclaration ||
...
function isBlockScopedVariableDeclarationList(declarationList) {
    return (
        (declarationList.flags & ts9__default.default.NodeFlags.BlockScoped) !==
        0
    );
}
function isNamespaceExported(node) {
    return (
        node.parent.kind ===
            ts9__default.default.SyntaxKind.ModuleDeclaration ||
        includesModifier(
            node.modifiers,
            ts9__default.default.SyntaxKind.ExportKeyword
        )
