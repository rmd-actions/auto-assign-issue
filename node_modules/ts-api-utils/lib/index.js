        ...
        (token) => {
            ...
            if (token.kind !== ts9.SyntaxKind.JsxText) {
                ts9.forEachLeadingCommentRange(
                    ...
                    token.pos === 0
                        ? (ts9.getShebang(fullText) ?? '').length
                        : token.pos,
                    commentCallback
                );
            }
        ...
        case ts9.SyntaxKind.ExpressionWithTypeArguments:
            return parent.parent.token === ts9.SyntaxKind.ExtendsKeyword &&
                parent.parent.parent.kind !==
                    ts9.SyntaxKind.InterfaceDeclaration
                ? 1 /* Read */
                : 0 /* None */;
        ...
        switch (node.parent.kind) {
            ...
            case ts9.SyntaxKind.SpreadElement:
                if (
                    ...
                    ts9.SyntaxKind.ArrayLiteralExpression
                ) {
                    return false;
                }
                node = node.parent.parent;
...
function isArrayBindingOrAssignmentPattern(node) {
    return (
        ts9.isArrayBindingPattern(node) || ts9.isArrayLiteralExpression(node)
    );
}
...
function isEvolvingArrayType(type) {
    return (
        isObjectType(type) &&
        isObjectFlagSet(type, ts9.ObjectFlags.EvolvingArray)
    );
}
...
function isTypeReference(type) {
    return (
        isObjectType(type) && isObjectFlagSet(type, ts9.ObjectFlags.Reference)
    );
}
...
function symbolHasReadonlyDeclaration(symbol, typeChecker) {
    return !!(
        (symbol.flags & ts9.SymbolFlags.Accessor) ===
            ts9.SymbolFlags.GetAccessor ||
...
var UsageWalker = class {
    ...
    #handleFunctionLikeDeclaration(node, cb, varCb) {
        ...
        const scope = (this.#scope =
            node.kind === ts9.SyntaxKind.FunctionExpression &&
            node.name !== undefined
